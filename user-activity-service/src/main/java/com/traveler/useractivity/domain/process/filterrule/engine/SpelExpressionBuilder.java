package com.traveler.useractivity.domain.process.filterrule.engine;
...
public final class SpelExpressionBuilder {
    ...
    private SpelExpressionBuilder() {
        throw new UnsupportedOperationException("This is a utility class and cannot be instantiated");
    }

    public static String build(List<FilterRuleNode.Element> conditions) {
        if (conditions == null || conditions.isEmpty()) return "";

        StringBuilder spel = new StringBuilder();

        for (FilterRuleNode.Element element : conditions) {
            switch (element) {
                case FilterRuleNode.Condition cond -> appendCondition(spel, cond);
                case FilterRuleNode.Operator op -> spel.append(" ")
                        .append(op.value().getValue())
                        .append(" ");
                case FilterRuleNode.Paren paren -> spel.append("left-paren".equals(paren.type()) ? "(" : ")");
    ...
    private static void appendCondition(StringBuilder spel, FilterRuleNode.Condition cond) {
